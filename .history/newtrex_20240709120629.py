...
def extract_product_data(html_content):
    ...
    for item in soup.select('.box.browsingitem'):
        ...
        if desc_tag:
            ...
            product_details = {
                "RAM": re.search(r'RAM\s+(\d+GB)', desc_tag.get_text(strip=True)).group(1) if re.search(r'RAM\s+(\d+GB)', desc_tag.get_text(strip=True)) else None,
                "GPU": re.search(r'GPU,\s+([\w\s\d\-]+)', desc_tag.get_text(strip=True)).group(1) if re.search(r'GPU,\s+([\w\s\d\-]+)', desc_tag.get_text(strip=True)) else None,
                "SSD": re.search(r'SSD\s+(\d+GB)', desc_tag.get_text(strip=True)).group(1) if re.search(r'SSD\s+(\d+GB)', desc_tag.get_text(strip=True)) else None
            }
