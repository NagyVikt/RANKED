...
category_id = 102
total_pages = 2  # This is the variable you can redefine each run
...
def extract_product_info_from_category_page(category_url):
    ...
    for product_card in soup.select('div.product-card-main'):
        ...
        if link_tag:
            product_url = f"https://www.svx.sk{link_tag['href']}"
...
def fetch_additional_details(product_info):
    ...
    attributes = []
    attribute_tags = soup.find_all('div', class_='attribute-class')
    for attribute in attribute_tags:
        attributes.append(attribute.get_text().strip())
    additional_details['attributes'] = attributes

    image_tag = soup.find('img', {'class': 'product-image-class'})
    if image_tag:
        additional_details['image'] = image_tag['src']

    description_tag = soup.find('div', {'class': 'description-class'})
    if description_tag:
    ...
    if regular_price_tag:
        ...
        additional_details['price'] = f"{regular_price}"

    discount_tag = soup.find('span', class_='text-brand')
    if discount_tag:
        ...
        if sale_price_tag:
            ...
            additional_details['price'] = f"{sale_price} €"
    else:
        additional_details['sale_price'] = 0
