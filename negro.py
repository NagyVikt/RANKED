...
def fetch_additional_details(product_url):
    ...
    # Extracting súly
    suly_match = re.search(r'Súly:\s*(\d+,\d+ g)', additional_details['description'])
    if suly_match:
        additional_details['suly'] = suly_match.group(1)
        print(f"Extracted súly: {additional_details['suly']}")
    else:
    ...
    for i, product in enumerate(products):
        ...
        if product_url:
            ...
            if additional_details:
                ...
                # Ensure weight (súly) is included in the attributes
                if 'suly' in additional_details:
                    product['attributes'].append(f"Súly: {additional_details['suly']}")
